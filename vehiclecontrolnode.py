...
class VehicleControlNode(Node):
    ...
    def red_light_callback(self, msg):
        # Set red_light_detected based on incoming data
    ...
    def controlLoop(self):
        ...
        if self.red_light_detected:
            ...
            # If red light is not detected, continue moving towards waypoints
        ...
        if self.stop_detected:
            u = 0
            self.qcar.write(u, 0)  # Stop the car
            self.get_logger().info(f"Stopped due to stop sign. Time since stop: {time.time() - self.stop_time:.2f} seconds.")
...
def main(args=None):
    ...
    try:
        # Runs the node, handling events with spin_once
        while rclpy.ok():
...
class VehicleControlNode(Node):
    def __init__(self, initialPosition, initialOrientation, nodeSequence):
        ...
        self.stop_time = None  # Track the time when the stop sign was detected
        self.red_light_detected = False  # Track if red light is detected
        self.u = 0  # Store throttle value
        self.delta = 0  # Store steering angle
    ...
    def controlLoop(self):
        ...
        if time.time() - self.t0 >= self.startDelay:
            self.u = self.speedController.update(v, self.v_ref, self.dt)
            self.delta = self.steeringController.update(p, th, v)
            self.qcar.write(self.u, self.delta)
            
        # Control the QCar based on the red light status
        if (self.red_light_detected == False):
            ...
            if is_second == True:
                # Stop the car if red light is detected, save current `u` and `delta`
                self.u = 0
                self.delta = 0
                self.qcar.write(self.u, self.delta)
                self.get_logger().info("Red light detected: Stopping the vehicle.")
                ...
                #if (self.red_light_detected == False):
                    if time.time() - self.t0 >= self.startDelay:
                        # Restore the previous u and delta values
                        self.u = self.speedController.update(v, self.v_ref, self.dt)
                        self.delta = self.steeringController.update(p, th, v)
                        self.qcar.write(self.u, self.delta)
